...
- [~] Recruit 12+ real testers, opt them into Closed Testing
  ...
  - Email infra pivot (2026-04-24): earlier welcome-email test to `welcometothebestplace@gmail.com` hit Resend's "cloudbot-ai.com domain not verified" wall (only the account-owner email could receive without domain verification). Fix: deleted unused myfeed.space from Resend's free-tier slot, added gethandyai.app as the new verified domain. DNS records (DKIM TXT `resend._domainkey`, SPF TXT on `send`) added to Namecheap; DNS-dig confirms live propagation. Also refactored signup to a dedicated `handy_beta_signups` table (earlier idempotency bug: unique-email constraint on shared waitlist_entries 500'd repeat signups). **Open issue:** Namecheap doesn't support MX records in Host Records — MX-for-`send` requires switching Mail Settings to Custom MX which would break the existing `hi@gethandyai.app` email forwarding. Testing whether Resend will verify with DKIM+SPF TXT alone.
- [ ] Run Closed Testing for 14 days with ≥12 testers before "Apply for Production" unlocks
- [x] (Deadline Sept 2026) Register sideloaded `docs/HandyAI.apk` on Android developer verification page
  - Done 2026-04-24: added SHA-256 fingerprint `A7:C8:3E:33...61:9C` from `app/handy-ai-release.jks` via Play Console → Android developer verification → Handy AI (com.claudeagent.phone) → Add key. Status: "In review" (auto-verifies within ~24h). Google's Play Store re-signing key `DE:CD:3F:C2...E6:4A` pre-existed as "Verified" (auto-registered Apr 22). Both keys on file so the direct-install APK stays installable on certified Android devices past Sept 2026.
